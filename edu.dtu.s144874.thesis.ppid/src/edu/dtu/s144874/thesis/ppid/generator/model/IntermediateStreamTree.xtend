...
class IntermediateStreamTree {
	...
	def addGroupedInputs(java.util.Map<org.eclipse.emf.ecore.EObject, java.util.List<org.eclipse.xtext.util.Pair<edu.dtu.s144874.thesis.ppid.ppid.Property, edu.dtu.s144874.thesis.ppid.ppid.Property>>> groupedInputs) {
		groupedInputs
		.forEach[key, info| this.add(new IntermediateStreamLeaf(key, info.map[new GroupingInformation(key.name, it.first, it.second,key.leafType)]))]
	}
...
//	}
	...
	def addPredicateSource(edu.dtu.s144874.thesis.ppid.ppid.Source source, java.util.List<edu.dtu.s144874.thesis.ppid.ppid.Predicate> predicates) {
